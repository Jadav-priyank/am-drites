    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.06);border:1px solid #e5e7eb;">
        ...
        <tr><td style="background:#fef3c7;padding:14px 20px;text-align:center;border-top:1px solid #fde68a;">
          <p style="color:#92400e;font-size:12px;margin:0;line-height:1.5;">Questions? Contact customer care at <strong>info@amdriets.com</strong></p>
        </td></tr>
      </table>
    </td></tr>
  </table>
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.06);border:1px solid #e5e7eb;">
        ...
        <tr><td style="background:#f8fafc;padding:14px 20px;text-align:center;border-top:1px solid #e2e8f0;">
          <p style="color:#64748b;font-size:12px;margin:0;">Thank you for shopping with AM DRIETS · info@amdriets.com</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendAdminNewOrderEmail(order, userEmail, userName) {
  const adminEmail = process.env.ADMIN_EMAIL || 'admin@amdriets.com';

  if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.log(`[DEV EMAIL] Admin New Order notification email for order ${order._id} to ${adminEmail}`);
    return;
  }
  ...
  try {
    ...
    const addressHtml = address ? `
      <div style="background:#f8fafc;border:1px solid #e2e8f0;padding:16px;border-radius:12px;margin:18px 0;text-align:left;">
        <p style="color:#1e293b;font-size:13px;font-weight:700;margin:0 0 6px;">Customer & Delivery Details</p>
        <p style="color:#334155;font-size:13px;margin:0 0 3px;"><strong>Name:</strong> ${address.name || userName || 'N/A'}</p>
        <p style="color:#334155;font-size:13px;margin:0 0 3px;"><strong>Email:</strong> ${userEmail || 'N/A'}</p>
        <p style="color:#334155;font-size:13px;margin:0 0 3px;"><strong>Phone:</strong> ${address.phone || 'N/A'}</p>
        <p style="color:#475569;font-size:13px;margin:4px 0 0;line-height:1.5;"><strong>Address:</strong> ${address.addressLine1}${address.addressLine2 ? ', ' + address.addressLine2 : ''}, ${address.city}, ${address.state} - ${address.pinCode}</p>
      </div>
    ` : `
      <div style="background:#f8fafc;border:1px solid #e2e8f0;padding:16px;border-radius:12px;margin:18px 0;text-align:left;">
        <p style="color:#334155;font-size:13px;margin:0;"><strong>Customer Name:</strong> ${userName || 'N/A'}</p>
        <p style="color:#334155;font-size:13px;margin:4px 0 0;"><strong>Customer Email:</strong> ${userEmail || 'N/A'}</p>
      </div>
    `;
    ...
    await transporter.sendMail({
      from: process.env.SMTP_FROM || `"AM DRIETS" <${process.env.SMTP_USER}>`,
      to: adminEmail,
      subject: `🚨 New Order Received! - ${orderIdShort} (₹${order.totalAmount})`,
      html: `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body style="margin:0;padding:16px 8px;background:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:540px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.06);border:1px solid #e5e7eb;">
        <tr><td style="background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%);padding:28px 20px;text-align:center;">
          <h1 style="color:#ffffff;margin:0;font-size:24px;font-weight:900;letter-spacing:-0.5px;">AM DRIETS</h1>
          <p style="color:#f97316;margin:4px 0 0;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1px;">🚨 New Order Received</p>
        </td></tr>
        <tr><td style="padding:28px 20px;text-align:center;">
          ...
          <h2 style="color:#111827;font-size:20px;font-weight:800;margin:0 0 8px;">Order Notification</h2>
          <p style="color:#4b5563;font-size:14px;margin:0 0 16px;line-height:1.6;">
            A new order <strong>${orderIdShort}</strong> has been successfully placed on the store.
          </p>
          ...
          <div style="border-top:2px solid #f97316;padding-top:14px;margin-top:14px;text-align:right;">
            ...
            <p style="font-size:18px;font-weight:800;color:#ea580c;margin:0;">Total Value: ₹${order.totalAmount}</p>
          </div>
        </td></tr>
        <tr><td style="background:#f8fafc;padding:14px 20px;text-align:center;border-top:1px solid #e2e8f0;">
          <p style="color:#64748b;font-size:12px;margin:0;font-weight:600;">AM DRIETS Order Management System · info@amdriets.com</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
    ...
    console.error('Send Admin New Order Email Error:', err);
  }
}

export async function sendOrderConfirmationEmail(order, userEmail, userName) {
  // Always trigger admin notification alongside customer email
  sendAdminNewOrderEmail(order, userEmail, userName).catch(err => console.error("Admin order notification email error:", err));
  ...
  try {
    ...
    await transporter.sendMail({
      from: process.env.SMTP_FROM || `"AM DRIETS" <${process.env.SMTP_USER}>`,
      to: userEmail,
      subject: `Your Order is Placed Successfully! - ${orderIdShort}`,
      html: `
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.06);border:1px solid #e5e7eb;">
        ...
        <tr><td style="padding:28px 20px;text-align:center;">
          <div style="width:48px;height:48px;background:#fff7ed;border-radius:50%;margin:0 auto 16px;line-height:48px;font-size:24px;color:#f97316;">🎉</div>
          <h2 style="color:#111827;font-size:20px;font-weight:800;margin:0 0 10px;">Your Order is Placed Successfully!</h2>
          <p style="color:#4b5563;font-size:14px;margin:0 0 16px;line-height:1.6;">
            Hi <strong style="color:#111827;">${userName || 'Customer'}</strong>,<br/>
            Thank you for shopping with AM DRIETS! Your order <strong style="color:#f97316;">${orderIdShort}</strong> has been placed successfully and is being prepared for dispatch.
          </p>
        ...
        <tr><td style="background:#fff7ed;padding:14px 20px;text-align:center;border-top:1px solid #ffedd5;">
          <p style="color:#9a3412;font-size:12px;margin:0;line-height:1.5;">Need assistance? Reply to this email or reach us at <strong>info@amdriets.com</strong></p>
        </td></tr>
      </table>
    </td></tr>
  </table>
